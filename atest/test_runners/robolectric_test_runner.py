...
EVENT_RE = re.compile(
    r'^(?P<event_name>[A-Z_]+) (?P<json_data>{(.\r*|\n)*})(?:\n|$)'
)
...
class RobolectricTestRunner(test_runner_base.TestRunnerBase):
  ...
  def run_tests_raw(self, test_infos, extra_args, reporter):
    ...
    for test_info in test_infos:
      full_env_vars = self._get_full_build_environ(test_info, extra_args)
      run_cmd = self.generate_run_commands([test_info], extra_args)[0]
      subproc = self.run(
          run_cmd, output_to_stdout=self.is_verbose, env_vars=full_env_vars
      )
  ...
  def run_tests_pretty(self, test_infos, extra_args, reporter):
    ...
    for test_info in test_infos:
      ...
      with tempfile.NamedTemporaryFile(dir=self.results_dir) as event_file:
        ...
        full_env_vars = self._get_full_build_environ(
            test_info, extra_args, event_file
        )
        run_cmd = self.generate_run_commands([test_info], extra_args)[0]
        subproc = self.run(
            run_cmd, output_to_stdout=self.is_verbose, env_vars=full_env_vars
        ...
        self.handle_subprocess(
            subproc,
            partial(
                self._exec_with_robo_polling, event_file, subproc, event_handler
            ),
        )
        ret_code |= self.wait_for_subprocess(subproc)
    if not ret_code:
  ...
  def _get_full_build_environ(
      self, test_info=None, extra_args=None, event_file=None
  ):
    ...
    Args:
        ...
        extra_args: Dict of extra args to add to test run.
        event_file: A file-like object that can be used as a temporary storage
          area.
    """
    full_env_vars = os.environ.copy()
    env_vars = self.generate_env_vars(test_info, extra_args, event_file)
  ...
  def _exec_with_robo_polling(
    ...
    Args:
        ...
        event_handler: A file-like object storing the events of robolectric
          tests.
    """
    buf = ''
    while True:
      ...
      if not reg.match(buf) or data == '':
        ...
        for event in re.split(r'\n\n', buf):
          ...
          if match:
            try:
              event_data = json.loads(match.group('json_data'), strict=False)
            except ValueError:
              # Parse event fail, continue to parse next one.
              logging.debug(
                  '"%s" is not valid json format.', match.group('json_data')
              )
              continue
  ...
  def generate_env_vars(test_info, extra_args, event_file=None):
    ...
    if filters:
      ...
      if robo_filter.methods:
        logging.debug(
            'method filtering not supported for robolectric tests yet.'
        )
    if event_file:
  ...
  def _check_robo_tests_result(test_infos):
    ...
    for test_info in test_infos:
      result_output = Path(
          os.getenv(constants.ANDROID_PRODUCT_OUT, '')
      ).joinpath(
          f'obj/ROBOLECTRIC/{test_info.test_name}_intermediates/output.out'
      )
        ...
        with result_output.open() as f:
          for line in f.readlines():
            if str(line).find('FAILURES!!!') >= 0:
              logging.debug(
                  '%s is failed from %s', test_info.test_name, result_output
              )
