...
class SuitePlanTestRunnerUnittests(unittest.TestCase):
  ...
  def test_generate_run_commands(self, mock_resultargs):
    ...
    run_cmd.append(
        suite_plan_test_runner.SuitePlanTestRunner._RUN_CMD.format(
            exe=exe_cmd, test=suite_name, args=''
        )
    )
    mock_resultargs.return_value = []
    unittest_utils.assert_strict_equal(
        self, self.suite_tr.generate_run_commands(test_infos, ''), run_cmd
    )
    ...
    run_cmd.append(
        suite_plan_test_runner.SuitePlanTestRunner._RUN_CMD.format(
            exe=exe_cmd, test=suite_name, args='--serial LG123456789'
        )
    )
    unittest_utils.assert_strict_equal(
        self,
        self.suite_tr.generate_run_commands(
            test_infos, {'SERIAL': 'LG123456789'}
        ),
        run_cmd,
    )
