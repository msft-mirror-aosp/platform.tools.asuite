...
class ModuleInfo:
    ...
    def __init__(self, force_build=False, module_file=None):
        ...
        self.name_to_module_info = self._merge_dependency(name_to_module_info)
    ...
    def _merge_dependency(self, name_to_module_info, dep_file_path=None):
        """Merge the dependency in dep_file_path to name_to_module_info.

        Args:
            name_to_module_info: Dict of module name to module info dict.
            def_file_path: String of path to file to load up. Used for testing.

        Returns:
            Dict of merged json of input def_file_path and name_to_module_info.
        """
        if not dep_file_path:
            dep_file_path = os.path.join(atest_utils.get_build_out_dir(),
                                         'soong', _JAVA_DEP_INFO)
        if not os.path.isfile(dep_file_path):
            return name_to_module_info
        try:
            with open(dep_file_path) as json_file:
                dep_infos = json.load(json_file)
        except json.JSONDecodeError:
            return name_to_module_info
        for module_name, dep_info in dep_infos.items():
            if name_to_module_info.get(module_name, None):
                dependencies = dep_info.get(constants.MODULE_DEPENDENCIES, [])
                mod_info = name_to_module_info.get(module_name)
                mod_info_dependencies = mod_info.get(
                    constants.MODULE_DEPENDENCIES, [])
                for dependency in dependencies:
                    if dependency not in mod_info_dependencies:
                        mod_info_dependencies.append(dependency)
                mod_info_dependencies.sort()
                name_to_module_info[module_name][
                    constants.MODULE_DEPENDENCIES] = mod_info_dependencies
