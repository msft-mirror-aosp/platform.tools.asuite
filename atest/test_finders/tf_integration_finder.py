...
class TFIntegrationFinder(test_finder_base.TestFinderBase):
  ...
  def _get_integration_dirs(self):
    ...
    """
    tf_dirs = list(
        filter(None, [d for x in _TF_TARGETS for d in self._get_mod_paths(x)])
    )
    gtf_dirs = list(
        filter(None, [d for x in _GTF_TARGETS for d in self._get_mod_paths(x)])
    )
  ...
  def _get_build_targets(self, rel_config):
    ...
    targets = test_finder_utils.get_targets_from_xml_root(
        xml_root, self.module_info
    )
    if self.gtf_dirs:
  ...
  def _load_include_tags(self, root):
    ...
    while i < len(root):
      ...
      if elem.tag == 'include':
        ...
        if node is None:
          raise atest_error.FatalIncludeError(
              "can't load %r" % integration_name
          )
        node = copy.copy(node)
        if elem.tail:
  ...
  def _search_integration_dirs(self, name):
    ...
    for integration_dir in self.integration_dirs:
      ...
      found_test_files = test_finder_utils.run_find_cmd(
          test_finder_utils.TestReferenceType.INTEGRATION, abs_path, name
      )
      if found_test_files:
      ...
      with ZipFile(prebuilt_jar, 'r') as jar_file:
        ...
        if xml_path in jar_contents:
          ...
          if not os.path.exists(extract_path):
            logging.debug('Extracting %s to %s', prebuilt_jar, extract_path)
  ...
  def _get_test_info(self, name, test_file, class_name):
    ...
    if int_name != name:
      logging.debug(
          'Input (%s) not valid integration name, did you mean: %s?',
          name,
          int_name,
      )
      return None
    ...
    if class_name:
      ...
      if '.' in class_name:
        ...
        logging.debug(
            'Looking up fully qualified class name for: %s.'
            'Improve speed by using fully qualified names.',
            class_name,
        )
        paths = test_finder_utils.find_class_file(self.root_dir, class_name)
        if not paths:
          return None
        for path in paths:
          class_name = test_filter_utils.get_fully_qualified_class_name(path)
          test_filters.append(test_info.TestFilter(class_name, methods))
  ...
  def find_int_test_by_path(self, path):
    ...
    if not os.path.exists(path):
      ...
      return None
    int_dir = test_finder_utils.get_int_dir_from_path(
        path, self.integration_dirs
    )
    if int_dir:
