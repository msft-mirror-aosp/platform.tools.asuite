...
class SDKConfig():
    ...
    Attributes:
        config_file: The absolute file path of the jdk.table.xml, the file
                     might not exist.
        template_jdk_file: The file of JDK table which record JDK info in
                           IDE.
    ...
    def _get_first_element_value(self, node, element_name):
        ...
        return elements[0].getAttribute(self._TAG_VALUE) if elements else None

    def _target_jdk_exists(self):
        """Check if the JDK18 already set in jdk.table.xml.

        Returns:
            Boolean: True if the JDK18 exists else False.
        """
        jdks = self.xml_dom.getElementsByTagName(self._TAG_JDK)
        for jdk in jdks:
    ...
    def _append_jdk_config(self, new_jdk_config):
    ...
    def generate_jdk_config(self):
        """Generate the content of jdk confiduration."""
        # Add in the JDK18 configuration at the end of the <component> if the
        # config file exists but the JDK18 doesn't exist.
        if self.config_exists and not self._target_jdk_exists():
            self._append_jdk_config(self._load_template())
        self.config_string = self.config_string.replace(self._JDK_PATH_TOKEN,
                                                        self.jdk_path)

    def set_jdk_config(self):
        ...
        if self.config_exists and self._target_jdk_exists():
            return
        self.generate_jdk_config()
        self._write_jdk_config()
